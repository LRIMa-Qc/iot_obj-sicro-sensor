...
int sht20_init()
{
	...
	// 	RET_IF_ERR(i2c_write_dt(&sht20_spec, cmdBuff, 1), "calibration initialization failed");
	// 	k_sleep(K_MSEC(10));
	// }
...
int sht20_read(float *temperature, float *humidity)
{
    ...
    RETRY_IF_ERR(i2c_write_dt(&sht20_spec, cmdBuff, 1), "trigger measure failed");

    k_sleep(K_MSEC(60));
    while (1) {
        RET_IF_ERR(i2c_read_dt(&sht20_spec, dataBuff, 3), "read failed");
        /* Check if the data is ready */
        if(dataBuff[3]) k_sleep(K_MSEC(5)); 
        else break;
    }
