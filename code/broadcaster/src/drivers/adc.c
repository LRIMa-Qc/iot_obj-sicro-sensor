...
int adc_init() {
    ...
    LOG_IF_ERR(gpio_pin_set_dt(&pt19_enable_spec, 0), "Luminosity GPIO pin set failed");
...
int ground_humidity_read(float *humidity) {
    ...
    LOG_IF_ERR(gpio_pin_set_dt(&hum_enable_spec, 1), "Ground humidity GPIO pin set failed");
    k_sleep(K_MSEC(40)); /* Wait for the sensor to be ready */

    /* Read the value */
    LOG_IF_ERR(adc_sequence_init_dt(&hum_adc_spec, &sequence), "Ground humidity ADC sequence init failed");
    LOG_IF_ERR(adc_read(hum_adc_spec.dev, &sequence), "Ground humidity ADC read failed");
...
int ground_temperature_read(float *temperature) {
    ...
    /* Activate power to the sensor */
    LOG_IF_ERR(gpio_pin_set_dt(&temp_enable_spec, 1), "Ground temperature GPIO pin set failed");
    k_sleep(K_MSEC(40)); /* Wait for the sensor to be ready */

    /* Read the value */
    LOG_IF_ERR(adc_sequence_init_dt(&temp_adc_spec, &sequence), "Ground temperature ADC sequence init failed");
    LOG_IF_ERR(adc_read(temp_adc_spec.dev, &sequence), "Ground temperature ADC read failed");
...
int luminosity_read(float *luminosity) {
    ...
    /* Activate power to the sensor */
    LOG_IF_ERR(gpio_pin_set_dt(&pt19_enable_spec, 1), "Luminosity GPIO pin set failed");
    k_sleep(K_MSEC(10)); /* Wait for the sensor to be ready */

    /* Read the value */
    LOG_IF_ERR(adc_sequence_init_dt(&pt19_adc_spec, &sequence), "Luminosity ADC sequence init failed");
    LOG_IF_ERR(adc_read(pt19_adc_spec.dev, &sequence), "Luminosity ADC read failed");
...
int battery_voltage_read(float *voltage) {
    ...
    /* Read the value */
    LOG_IF_ERR(adc_sequence_init_dt(&bat_adc_spec, &sequence), "Battery voltage ADC sequence init failed");
    LOG_IF_ERR(adc_read(bat_adc_spec.dev, &sequence), "Battery voltage ADC read failed");

    uint32_t millivolts = sample_buffer;
    LOG_IF_ERR(adc_raw_to_millivolts_dt(&bat_adc_spec, &millivolts), "Battery voltage ADC raw to millivolts failed");
